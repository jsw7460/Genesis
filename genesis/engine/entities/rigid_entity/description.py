...
class KinematicEntityDescription(EntityDescription):
    ...
    # Link-name pairs an MJCF '<contact><exclude>' forbids from colliding. The collider filters them beside the
    # welded pairs, so the contype / conaffinity masks stay as the file wrote them (see 'parse_geoms' in mjcf.py).
    excluded_links_name: list[tuple[str, str]] = field(default_factory=list)
        ...
        for morph in self.morphs[1:]:
            if isinstance(morph, (gs.morphs.URDF, gs.morphs.MJCF)):
                ...
                if {frozenset(pair) for pair in v_excluded_links_name} != {
                    frozenset(pair) for pair in self.excluded_links_name
                }:
                    gs.raise_exception(
                        f"Heterogeneous variant declares contact exclusions {v_excluded_links_name}, but primary "
                        f"declares {self.excluded_links_name}. All variants must declare the same exclusions."
                    )
    ...
    def _parse_scene(self, morph, resolution: Resolution):
        ...
        if isinstance(morph, gs.morphs.MJCF):
            ...
            l_infos, links_j_infos, links_g_infos, eqs_info, excluded_links_name = mju.parse_xml(
                morph, self.surface, resolution.options
            )
            ...
            try:
                ...
                l_infos_mj, links_j_infos_mj, links_g_infos_mj, _, _ = mju.parse_xml(morph_, self.surface)
    ...
    def _load_scene(self, morph, resolution: Resolution):
        l_infos, links_j_infos, links_g_infos, _eqs_info, self.excluded_links_name = self._parse_scene(
            morph, resolution
        )
...
class RigidEntityDescription(KinematicEntityDescription):
    ...
    def _load_scene(self, morph, resolution: Resolution):
        l_infos, links_j_infos, links_g_infos, eqs_info, self.excluded_links_name = self._parse_scene(
            morph, resolution
        )
